"""Create the deterministic, final OSRM-routable cohort from the test split."""
...
ROUTE_ESTIMATES_PATH = OSRM_DIRECTORY / "test-route-estimates.parquet"
METADATA_PATH = OSRM_DIRECTORY / "test-route-metadata.json"
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build Cabrynt's deterministic final OSRM test cohort."
    )
...
def _reject_existing_output() -> None:
    ...
    if existing_paths:
        raise FileExistsError(
            "The final test cohort already exists and must not be replaced: "
            f"{existing_paths}"
        )
